...
const rickAndMortyCharacters:RickAndMortyCharacter[] = [
    {
        "id": 1,
        "name": "Rick Sanchez",
        "status": "Alive",
...
function findAllLivingHumans(characters: RickAndMortyCharacter[]){
    return characters.filter((character) => character.status === 'Alive' && character.species === 'Human');
}

function printRickAndMortyCharacters(characters: RickAndMortyCharacter[]){
    characters
        .map((character) => " ".repeat(4-character.id.toString().length) + character.id + ":  " + character.name)
        .forEach((name) => console.log(name));
}

printRickAndMortyCharacters(findAllLivingHumans(rickAndMortyCharacters));

function namesOfTheCharacters(characters: RickAndMortyCharacter[]){
...
function namesAndOrigins(characters: RickAndMortyCharacter[]): NameOrigin[]{
